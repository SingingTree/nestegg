  ...
  *data = NULL;
  ...
  if (entry->content_encodings.content_encoding.head) {
    ...
    if (encoding_type == NESTEGG_ENCODING_ENCRYPTION) {
      ...
      if (ne_get_uint(aes_settings->aes_settings_cipher_mode, &encryption_mode) != 0)
        return -1;
  ...
  for (i = 0; i < frames; ++i) {
    ...
    /* Parse encryption */
    if (encoding_type == NESTEGG_ENCODING_ENCRYPTION) {
      ne_io_read(ctx->io, &signal_byte, SIGNAL_BYTE_SIZE);
      ...
      if((signal_byte & ENCRYPTED_BIT_MASK) == PACKET_ENCRYPTED) {
        ...
        ne_io_read(ctx->io, f->frame_encryption->iv, IV_SIZE);
...
nestegg_packet_encryption(nestegg_packet * pkt, unsigned char ** iv, size_t * length)
{
  ...
  /* Encryption does not support lacing, should have no next frame */
  assert(f->next == NULL);
...
int
nestegg_track_encryption(nestegg * context, unsigned int track,
  ...
  entry = ne_find_track_entry(context, track);
  if (!entry)
    return -1;
  ...
  /* At this stage we should have returned if encryption isn't present */
  if (!encoding->content_encryption.head)
    return -1;
